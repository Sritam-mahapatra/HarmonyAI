...
def load_rgb_frames_from_video():
    # sourcery skip: remove-unnecessary-else, simplify-boolean-comparison, swap-if-else-branches
    # sourcery skip: remove-unnecessary-else, simplify-boolean-comparison, swap-if-else-branches
    ...
    while True:
        ...
        if ret == True:
            ...
            if offset > batch:
                ...
                if offset % 20 == 0:
                    ...
                    if text != " ":
                        ...
                        word = language.get_suggestions(
                            text_list, n_gram_counts_list, vocabulary, k=1.0
                        )
                        if word != " .":
                            sentence += word
            ...
            cv2.putText(
                frame1, sentence, (120, 520), font, 0.9, (0, 255, 255), 2, cv2.LINE_4
            )
...
def load_model(weights, num_classes):
    ...
    nlp = pipeline(
        "k2t-new"
    )  # The pre-trained models available are 'k2t', 'k2t-base', 'mrm8488/t5-base-finetuned-common_gen', 'k2t-new'
    global params
    params = {
        "do_sample": True,
        "num_beams": 5,
...
def create_WLASL_dictionary():  # sourcery skip: hoist-statement-from-if
