...
class NGrams:
    ...
    def sentences(self) -> list:
        ...
        if self._sentences is None:
            self._sentences = [
                tuple(self.start_tokens + sentence + self.end_tokens)
    ...
    def n_grams(self) -> list:
        ...
        if self._n_grams is None:
            self._n_grams = chain.from_iterable(
                [
                    [
                        sentence[cut : cut + self.n]
                        for cut in range(0, len(sentence) - (self.n - 1))
                    ]
...
class Tokenizer:
    ...
    def tokenized(self) -> list:
        ...
        if self._tokenized is None:
            self._tokenized = [
                nltk.word_tokenize(sentence.lower()) for sentence in self.sentences
            ]
